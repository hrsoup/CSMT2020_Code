...
from.bilstm_rules import BiLSTM_RULES


class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size, crf=True,rules = True):
        ...
        if crf == True:
            ...
            if rules == True:
                self.model = BiLSTM_RULES(vocab_size, self.emb_size,
                                    self.hidden_size, out_size).to(self.device)
    ...
    def test(self, word_lists, tag_lists, word2id, tag2id):
        ...
        with torch.no_grad():
            batch_tagids = self.best_model.test(
                word_lists, tensorized_sents, lengths, tag2id)
...
class BiLSTM_CRF(nn.Module):
    ...
    def test(self, word_lists, test_sents_tensor, lengths, tag2id):
